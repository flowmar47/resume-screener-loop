...
- **Apply the anti-pattern checklist** before declaring the draft done. See `reference/anti-patterns.md`.

Generate the DOCX using the templates in `templates/`. The library is built on `docx-js` (Node) and produces professional-looking ATS-friendly output with sensible defaults (Calibri, 0.75" margins, US Letter, modest accent color).
...
- Apply the changes. Re-run the anti-pattern checklist.
- Re-verify page count (aim for two pages; tighten if longer).
- Spot-check the rendered PDF visually for at least one resume in a batch.
